...
class SQLDataReader(SQLHandlerMixin):
    ...
    def setup_connection(self, username = 'fx_admin', password = '#Flexstone2020'):
    ...
    def check_table_exist_or_not(self, schemas, table_name):
        ...
        sql = "IF (EXISTS (SELECT * FROM fund_clustering WHERE TABLE_SCHEMA = " + schemas + \
                 "AND TABLE_NAME = " + table_name + " )) BEGIN PRINT 'Table Exists' RETURN 1; END"
    ...
    def drop_table(self, schemas, table_name):
        ...
        sql = "DROP TABLE IF EXISTS " + table_name + " WHERE TABLE_SCHEMA = " + schemas
